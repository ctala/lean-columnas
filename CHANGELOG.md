...
- Breadcrumb navigation on single and archive templates.
- Two-column layout: article left, sticky sidebar right with related columns.
- Sidebar always shows "Ultimas columnas" section + author's columns if available.
- Safe deactivation: columnists/agencies demoted to subscriber with role stored in user meta.
- Safe reactivation: restores original roles from stored meta.
- Spanish translations for Submit for Review, Save Draft, Preview buttons.
- Custom Spanish post update messages for all column actions.
- `TAXONOMY_ENABLED` feature flag to disable categories until needed.

### Changed

- Removed featured image from CPT supports, templates, and quality gates.
- Author photo now uses user profile data (not featured image).
- All author photo CSS uses explicit width/height/aspect-ratio to prevent deformation.
- Archive cards no longer show featured images.
- Columnists can no longer edit published columns.

### Removed

- `checkFeaturedImage()` quality gate (dead code after featured image removal).
